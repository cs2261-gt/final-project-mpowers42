...
void playSoundA( const signed char* sound, int length, int loops) {
        ...
        // Assigns all of soundA’s appropriate struct values
        soundA.data = sound;
...
void playSoundB( const signed char* sound, int length, int loops) {
        ...
        REG_TM1CNT = TIMER_ON;
         ...
         // Assigns all of soundB’s appropriate struct values
        soundB.data = sound;
...
void interruptHandler() {
    ...
    if(REG_IF & INT_VBLANK) {
        if (soundA.isPlaying) {
            ...
            // Handles soundA playing in the interruptHandler function
            if (soundA.isPlaying) {
                soundA.vBlankCount++;
            }
        ...
        if (soundB.isPlaying) {
            ...
            // Handles soundB playing in the interruptHandler function
            if (soundB.isPlaying) {
                soundB.vBlankCount++;
            }
